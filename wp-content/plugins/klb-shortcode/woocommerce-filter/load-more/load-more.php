...
if(ot_get_option('groci_paginate_type') == 'loadmore' || groci_ft() == 'load-more'){
	function groci_load_more_button(){
		echo '<nav class="woocommerce-pagination klb-load-more">
				<div class="btn btn-secondary button light">'.esc_html__('Load More','groci-core').'</div>
			  </nav>';
	}

	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
	add_action( 'woocommerce_after_shop_loop', 'groci_load_more_button', 10 );
}
...
if(ot_get_option('groci_paginate_type') == 'infinite' || groci_ft() == 'infinite'){
	remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
}


/*************************************************
## Load More - Infinite Scripts
*************************************************/ 
if(ot_get_option('groci_paginate_type') == 'loadmore' || ot_get_option('groci_paginate_type') == 'infinite' || groci_ft() == 'load-more' || groci_ft() == 'infinite'){
	function groci_load_more_scripts() {
		if(is_shop() || is_product_category()){
			if(ot_get_option('groci_paginate_type') == 'infinite' || groci_ft() == 'infinite'){
				wp_enqueue_script( 'groci-load-more',  plugins_url( 'js/infinite-scroll.js', __FILE__ ), true );
			} elseif(ot_get_option('groci_paginate_type') == 'loadmore' || groci_ft() == 'load-more') {
				wp_enqueue_script( 'groci-load-more',  plugins_url( 'js/load_more.js', __FILE__ ), true );
			}
			wp_localize_script( 'groci-load-more', 'loadmore', array(
	...
	add_action( 'wp_enqueue_scripts', 'groci_load_more_scripts' );
}
/*************************************************
## Load More CallBack
*************************************************/ 

add_action( 'wp_ajax_nopriv_load_more', 'groci_load_more_callback' );
add_action( 'wp_ajax_load_more', 'groci_load_more_callback' );
function groci_load_more_callback() {
		...
		if ( $loop->have_posts() ) {
			...
			$output .= esc_html__( 'No products found','groci' );
		}
		wp_reset_postdata();
