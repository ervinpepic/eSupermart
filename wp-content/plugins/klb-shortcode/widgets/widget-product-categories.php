...
class klb_widget_klb_product_categories extends WP_Widget { 
	...
	function __construct() {
		$widget_ops = array('description' => esc_html__('For Main Shop Page.','groci') );
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'klb_product_categories' );
		 parent::__construct( 'klb_product_categories', esc_html__('Groci Product Categories','groci'), $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance) {
		if(is_shop()){
			...
			foreach ( $terms as $term ) {
				...
				echo '<li>';
				echo '<a href="'.esc_url(groci_get_cat_url($term->term_id)).'" class="product_cat">';
				...
				echo '</a>';
					if($term_children){
						...
						foreach($term_children as $child){
							...
							echo '<li>';
							echo '<a href="'.esc_url( groci_get_cat_url($childterm->term_id)).'">';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title:','groci'); ?></label>
			<input class="widefat"  id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('exclude'); ?>"><?php esc_html_e('Exclude id:','groci'); ?></label>
