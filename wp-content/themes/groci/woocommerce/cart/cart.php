...
<div class="card card-body cart-table">
	<div class="table-responsive">
		...
		<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
			...
			<table class="table cart_summary shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
				<thead>
					<tr>
						<th class="product-thumbnail">&nbsp;</th>
						<th class="product-name"><?php esc_html_e( 'Product', 'groci' ); ?></th>
						<th class="product-price"><?php esc_html_e( 'Price', 'groci' ); ?></th>
						<th class="product-quantity"><?php esc_html_e( 'Quantity', 'groci' ); ?></th>
						<th class="product-subtotal"><?php esc_html_e( 'Subtotal', 'groci' ); ?></th>
				...
				<tbody>
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
								...
								if ( ! $product_permalink ) {
									echo groci_sanitize_data($thumbnail); // PHPCS: XSS ok.
								} else {
								...
								<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'groci' ); ?>">
								<?php
								if ( ! $product_permalink ) {
								...
								if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
									echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'groci' ) . '</p>', $product_id ) );
								}
								?>
								</td>

								<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'groci' ); ?>">
									<?php
								...
								<td class="product-subtotal" data-title="<?php esc_attr_e( 'Subtotal', 'groci' ); ?>">
									<?php
					...
					<tr>
						<td colspan="6" class="actions klbaction">
